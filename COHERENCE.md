...
| `scripts/decision-alert` decision follow-through lifecycle | `scripts/compose-decision-prompt.py`, `dashboard/index.html`, `dashboard/panel.html`, `docs/runbooks/mission-control.md`, `openspec/specs/rollup-answer/spec.md`, and `STATE.md` | single and rollup answers stay non-terminal; exact receipt transitions and every operator-facing label must preserve `answered_pending -> delivered -> consumed -> running -> live_result_verified -> closed` |
