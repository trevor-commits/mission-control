...
| `scripts/loose-ends` | Pick up unfinished work: ranks the open-work ledger, attention board, `todo.md` Active Next Steps, and git state. `show`, `resolve`, and `prompt` provide source identity, freshness, and current repository facts while preserving the receiving task's model and reasoning. Paired with the `loose-ends` skill in `skills/`. |
