...
- 2026-08-12 terminal audit (Cursor `2bc6c15d`): live Git, PR, tag, bundle, install, and collector claims still hold. Fresh `cleanup-released` results are convergence `active-writer-lock`, Rollup Answer `active-writer-lock` plus `worktree-content-present` plus non-ancestor `5153e2d`, and Opus 5 `released-head-not-merged`. Collector runs advanced to 282 with last exit zero. Dashboard automation remains red from Nightly Review's 2026-08-11 empty Claude result, not from install drift. Full record: `records/2026-08-12-019ff2ae-terminal-audit.md`.
