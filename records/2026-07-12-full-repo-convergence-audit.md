...
- Current expanded focused usage suite: `PASS=22 FAIL=0` under `/bin/bash`; global usage-routing suite: `54 pass, 0 fail`; Mission/global collector source comparison, sanitized non-login scheduler execution, native ccusage argv, bounded-process cleanup, crash-safe kernel locking plus aged/future legacy migration, newest-event and future-time fail-closed handling, explicit-provider failure, zero-LLM scheduled execution, direct-argv plist contract, state-output failure, shell syntax, plist validation, bytecode-clean syntax gate, and `git diff --check`: PASS.
