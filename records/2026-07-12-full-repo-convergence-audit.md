...
| P2 | The checked parser still ignored a failed `tail` read, treating an unreadable or disappeared selected rollout as honest absence. | Capture and check the bounded source read before parsing; read failure emits two unknown/down Codex rows and a nonzero snapshot. |
...
| P1 | The exec fallback still silently returned Claude Haiku despite the same policy reserving Haiku/Sonnet for explicit down-tier overrides. | Use native Claude Opus as the conserved last-resort fallback and assert its exact model ID. |
| P1 | Router schema rejected the collector's valid `confidence=manual`, `health=held` purchased-credit rows even though routing later excluded credit inventory. | Permit `manual` confidence only on validated `credit:` windows and route a real collector-generated credit snapshot in the regression. |
| P2 | Spend-before-reset scoring examined only the max-used binding row, so a near-reset sibling could be ignored. | Compute one provider-level bonus when any eligible routing window resets in 0–180 minutes; cover near/far binding and nonbinding cases. |
...
| P2 | File-backed router snapshots accepted missing, invalid, stale, or materially future `fetched_at`, allowing old quota state to drive a current route. | Require parseable timestamps no older than one hour with at most five minutes future skew; every rejected timestamp exits 65. |
| P3 | Provider scores changed for unhealthy, uncertain, stale, or near-reset sibling windows without the reason naming those aggregate flags. | Include the exact disqualified, uncertain, stale, and spend-before-reset windows in ranked reasons and regression-check that near/far reasons differ. |
| P3 | `HISTORY_LOCK_FILE` remained assigned but unread after ownership moved to descriptor 9, falsely implying a path-dependent release invariant. | Remove the dead state from both byte-identical collectors; release remains exclusively kernel descriptor ownership. |
...
- Final focused suites: Mission `PASS=24 FAIL=0` under `/bin/bash`; global usage-routing `64 pass, 0 fail`. Mission/global/runtime source comparison, snapshot-freshness rejection, aggregate reason transparency, sanitized non-login scheduler execution, native ccusage argv, bounded source reads and parse failure, crash-safe kernel locking plus aged/future legacy migration, conservative multi-window routing, collector-compatible credit validation, row-schema validation, policy-compliant GPT-5.5/Opus defaults, provider-wide future-reset scoring, explicit-provider failure, zero-LLM scheduled execution, direct-argv plist contract, state-output failure, shell syntax, plist validation, bytecode-clean syntax gate, and `git diff --check`: PASS.
...
- Exact final Mission verifier at `c8a1e751c6c5cecfe199b68a561a4627850f21c0`: `SUITES PASS=21 FAIL=0`, dashboard `67/0`, ER-134 `50/0`, usage `24/0`, browser `253`; log `/tmp/mission-control-c8a1e75-verify.log`.
...
- Installed usage runtime: detached clean tree at the exact global SHA; collector SHA-256 `bb8b092ed1101ebdc1c85030ead70be4cc7fe05c639318ab4592349392b84423`; native ccusage 20.0.17 SHA-256 `08c455a4307345ca2b0fcda3a81edd9421a7edd53ea0acea19309925a7af54c0`; loaded direct-argv 1800-second LaunchAgent reached `runs=3`, exit `0`, proving RunAtLoad plus two natural recurrences. History/stdout advanced through 06:35 PDT, historical stderr remained unchanged, and no live probe, scoped provider/npm process, temp, or lock residue appeared.
