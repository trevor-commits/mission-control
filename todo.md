...
- ER-107 Morning Brief (highest, implemented-pending-proof): deterministic Tier 1 and bounded code-only Tier 2 outcomes, lineage-aware summaries, trust-separated decisions, resumable delivery code, installed dashboard, and default-dry-run loose-end review are review-clean. Next gates are an explicitly authorized Tier 2 provider sample/calibration, Telegram/deadman proof, launchd activation and natural-cadence proof, notification consolidation, and approximately five natural mornings before verification/archive. | owner: Codex thread `019f4963-1e75-7600-8a17-1e6f6f8e8ca6` | linear: repo-only; no Mission Control Linear team is configured.
