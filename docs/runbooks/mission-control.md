...
- **Corner panel:** `dashboard panel` installs `~/.mission-control/panel.html`, stages `~/.mission-control/Mission Control Panel.app` (LSUIElement), and launches the menu-bar app (`MC`). First launch may compile `scripts/mc-panel.swift` with `swiftc`. The panel disables AppKit automatic termination and holds a RunningBoard activity so Mission Control stays in the menu bar when idle.
