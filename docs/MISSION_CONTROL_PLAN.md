...
- Feeder-shape drift: privacy-safe specimens derived from observed live shapes are committed under `dashboard/fixtures/feeders/`, with all identity and content replaced by synthetic values (usage-snapshot.json, scan-unfinished-work.json, last-scan.tsv, delegation-state.json, mailbox-msg.json pinned to `schema:"cross-agent-mailbox/v1"`, session-index.jsonl, and Claude/Codex line samples). The dashboard and chat-graph acceptance suites run the real collectors against them so upstream drift fails loudly. Collectors wrap extraction failures → envelope `ok:false`, renderer keeps last-good + stale banner.
