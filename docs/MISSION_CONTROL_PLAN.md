...
- Feeder-shape drift: privacy-safe feeder-output and provider-record specimens derived from observed live shapes are committed under `dashboard/fixtures/feeders/`, with all identity and content replaced by synthetic values (usage-snapshot.json, scan-unfinished-work.json, last-scan.tsv, delegation-state.json, mailbox-msg.json pinned to `schema:"cross-agent-mailbox/v1"`, session-index.jsonl, and structurally authentic Claude/Codex records). The dashboard acceptance suite runs its real command-output wrapper against the usage/Git outputs; the chat-graph suite runs real delegation, mailbox, title, and transcript collectors against isolated provider stores. This locks the consumer boundary without falsely claiming to execute upstream generators from their own outputs. Extraction failures produce envelope `ok:false`; the renderer keeps last-good data with a stale banner.
