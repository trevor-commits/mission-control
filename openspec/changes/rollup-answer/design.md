...
7. **Publication and replay are recoverable.** Rollup prompts and manifests contain no wall-clock identity, so an exact replay reproduces the persisted digest. The artifact name bound to the held fd is authoritative independent of invocation lifecycle; a post-commit mutation, parent replacement, invalid existing/published directory, regular-file collision, or receipt-backed symlink collision is preserved under a private quarantine name without following a symlink target, and exact replay rebuilds the current deterministic batch without duplicating events. If a name changes after validation but before quarantine rename, the moved replacement is descriptor-verified and restored before failure is returned.
...
9. **Public success includes same-runtime local-view coherence.** The dashboard answer command binds its transaction composer and readers to one `SCRIPT_DIR` stack, ignores ambient `REPO_ROOT` and decision/brief feeder selectors for this transaction, runs the strict decisions collector with alert egress disabled, reconciles Morning Brief through that exact runtime, strictly republishes the brief feed, and returns nonzero while preserving the committed receipt for exact replay if any view cannot be refreshed. A delivered rewrite requires a mode-600 non-symlink receipt that contains a valid full Markdown digest and recorded chunking limit plus complete ordered chunk digests binding the sidecar-derived delivered bytes; neither identity field is optional. The first rewrite also pins the exact receipt-byte digest for later replay; receipt/cursor bytes remain immutable, and in-flight delivery states fail closed before calendar rollover is considered.
10. **Needs-you presentation preserves actionability.** Home and panel stably order actionable rows before answered-pending receipts before applying their display limits, so an actionable count cannot produce a `Needs you` heading with no visible actionable control. Home's global headline also derives from combined feed attention, not only the decision subset, so pending decision receipts cannot mask unrelated operator work.
...
| Receipt-backed canonical name is a non-directory on later replay | The exact regular file or symlink entry is opened without target traversal, inode-bound, and quarantined; an orphan first-answer conflict is never mutated |
| Canonical name changes between quarantine validation and rename | Post-rename identity mismatch is detected; the moved replacement is descriptor-bound and restored to its original canonical name before the transaction fails |
...
| Delivered receipt omits its Markdown digest or chunking limit | Local artifacts remain unchanged and the public command returns committed-but-refresh-failed nonzero without guessing from ambient defaults |
...
| All decisions are pending while another feed needs attention | Home's global headline remains `Needs you`; the decision card separately reports recorded/awaiting-consumption state |
