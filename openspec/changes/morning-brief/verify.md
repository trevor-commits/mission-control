...
- The final-gate hardening also makes feed/stamp wire values strict, required fresh inputs non-empty, install verification total, installed imports adjacent, source reads pinned to one immutable commit, browser/Python age-state boundaries identical, and plist deployment linted/atomic/checked with mode 0600 re-enforced even for unchanged bytes. Missing or invalid install provenance is red in status and deadman without creating an install-integrity Telegram category.
