...
- The live code/render stamp remains `1c6a380`. That is not the final reviewed candidate. The final-gate code is reconciled onto frozen activation commit `30f2f9f` as `3427d6b`; the full post-rebase cold/static matrix is green. The tree remains merge-held until the evidence update is committed and pushed, separate challengers audit one immutable SHA, that exact tree lands on canonical `main`, and it is reinstalled without a provider send.

## Open proof gates

1. Commit and push the reconciled final-gate evidence tree, obtain independent current-SHA review-clean evidence, land that exact tree on canonical `main`, and reinstall/verify its exact stamp and launchd state without provider delivery.
