...
- The live code/render stamp remains `1c6a380`. That is not the final reviewed candidate. The final-gate history is reconciled onto frozen activation commit `30f2f9f`; post-finding code commits `aba151b` and `00da629` and the full no-provider cold/static matrix are green. The published evidence tree remains merge-held until separate challengers audit one immutable SHA, that exact tree lands on canonical `main`, and it is reinstalled without a provider send.

## Open proof gates

1. Obtain independent current-SHA review-clean evidence, land that exact tree on canonical `main`, and reinstall/verify its exact stamp and launchd state without provider delivery.
