...
- Isolated cached Tier 2 outcome classification for high-value sessions, including a closed code taxonomy, strict local schema/cache/config validation, deterministic session/repo/lineage context, inferred-only follow-up lifecycle, global/provider kill switches, Haiku-to-Sonnet ambiguity escalation, OAuth-lock defer, budget fail-open, content-free health, and a 04:40 inactive LaunchAgent template ahead of the 05:00 brief.
...
- After `df991b4`, the normal installer refreshed the code-only runtime while default activation gates skipped Outcome Extractor, Morning Brief, and deadman. Full LaunchAgent SHA-256 inventories before/after were identical; no outcome-extractor plist appeared. The installed `dashboard extract-outcomes` path was then invoked with `MORNING_BRIEF_LLM=0` and returned zero calls (`uncalibrated_skips=1`), proving the baked repo-backed module path without transcript egress.
...
- The final-gate hardening also makes feed/stamp wire values strict, required fresh inputs non-empty, install verification total, installed imports adjacent, source reads pinned to one immutable commit, browser/Python age-state boundaries identical across tab banners, dots, named segments, and Home cards, and plist deployment linted/atomic/checked with mode 0600 re-enforced even for unchanged bytes. The committed demo fixture now uses the real Brief envelope cadence rather than relying on a smoke-only rewrite. Missing or invalid install provenance is red in status and deadman without creating an install-integrity Telegram category.
...
- Audit side effect: at 02:23:09 PDT, `/root/audit_faad_claims` used the wrong clock seam and its unstubbed deadman probe invoked the then-default mobile-connect sender, which returned rc 0; its report is preserved in Codex task `019f4f6c-e506-7551-876e-d9bb31f56c36`. It is therefore strongly inferred—not provider-receipt-confirmed—that Telegram received exactly `Morning Brief deadman: stale. Check Mission Control delivery.` No repo, runtime, or LaunchAgent file changed from that probe. All later transport checks used loopback or stubs.
- Audit side effect: `/root/immutable_code_audit` failed to create its intended launchctl stub during a plist-mode counterexample, so real launchctl replaced live `com.gillettes.mission-control` with a temporary plist that was subsequently removed. The governor immediately booted out that definition, validated and bootstrapped the canonical plist, and verified canonical path, `/bin/zsh`, `runs = 1`, and `last exit code = 0`. Runtime/plist bytes remained present, but the scheduler registration was temporarily disrupted.
- Final gate closure: post-finding code commit `00da629` closes the last accepted defects. Immutable evidence SHA `1261c48` received separate final code/runtime `READY` and records/provenance `review-clean` verdicts with no P0-P3 findings, exact local/tracking/remote identity, and a clean worktree. That exact tree was fast-forwarded to canonical `main`; the activation, final-gate, direct-sender, P9, and superseded hardening branches/worktrees were removed. The exact current-main closeout commit is installed and its five-runtime/two-asset stamp, source hashes, private plist modes, and canonical launchd registrations are verified before push. Provider delivery and gated Outcome Extractor activation remain off during this closeout.

## Open proof gates

1. Approve and measure a small privacy-screened provider sample, review content-free token/latency evidence, apply observed caps before any live Tier 2 backfill, and explicitly decide whether to activate the Outcome Extractor.
2. Record approximately five natural mornings of comprehension/action evidence, decide which older notifications to subsume/keep/fold, rerun the elapsed-proof audit, then mark the OpenSpec change verified and archive it.
