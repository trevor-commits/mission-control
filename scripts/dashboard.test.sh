...
c21() { # install stamps provenance from committed HEAD; verify detects runtime drift
  ...
  local n
  for n in dashboard chat-graph mission_control_common.py queue_admission.py morning-brief morning-brief-deadman decision-alert compose-decision-prompt.py mc-panel.swift self-repair usage-watch headroom-refresh; do
